...
The `rust/` directory contains a native port of the QF4 protocol core
(container, frame, compression, and RaptorQ transfer layers). It reproduces
the browser sender's stream byte-for-byte and is covered by wire-level interop
tests against fixtures generated by the JavaScript implementation. See
`rust/README.md` for details.
